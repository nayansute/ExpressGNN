...
class ConditionalMLN(nn.Module):
  ...
  def __init__(self, cmd_args, rule_list):
    ...
    self.rule_weights_lin = nn.DataParallel(self.rule_weights_lin)
  
  def forward(self, neg_mask_ls_ls, latent_var_inds_ls_ls, observed_rule_cnts, posterior_prob, flat_list,
              observed_vars_ls_ls):
    """
    Compute the MLN potential given the posterior probability of latent variables using lifted inference.
    ...
    for i in range(len(neg_mask_ls_ls)):
      ...
      for j in range(len(neg_mask_ls)):
        ...
        # Use lifted inference for Cartesian product
        cartesian_prod = self.lifted_cartesian_product(posterior_prob, latent_var_inds, latent_neg_mask)
  ...
  def lifted_cartesian_product(self, posterior_prob, latent_var_inds, latent_neg_mask):
    """
    Perform lifted Cartesian product for the given latent variables.

    :param posterior_prob: Posterior probabilities of latent variables.
    :param latent_var_inds: Indices of latent variables.
    :param latent_neg_mask: Mask indicating negation of latent variables.
    :return: Lifted Cartesian product.
    """
  ...
  def weight_update(self, neg_mask_ls_ls, latent_var_inds_ls_ls, observed_rule_cnts, posterior_prob, flat_list,
                    observed_vars_ls_ls):
    closed_world_potentials = torch.zeros(self.num_rules, dtype=torch.float)
    ...
    for i in range(len(neg_mask_ls_ls)):
      ...
      for j in range(len(neg_mask_ls)):
        ...
        has_pos_atom = any(val == 1 for val in observed_neg_mask + latent_neg_mask)
        if has_pos_atom:
          closed_world_potentials[i] += 1
        ...
        closed_world_potentials[i] += cartesian_prod.sum()

    return closed_world_potentials
